...
- [x] 12. OpenWeather 지역·위험·알림 구현
  Recommended task executor category: `deep`
  What to do / Must NOT do: 대략적 현재 위치의 지역 변환 또는 직접 지역 검색을 구현하고 수동 지역을 우선한다. Cloud Function이 OpenWeather의 기온·습도·강수·관측시각을 canonical snapshot으로 저장한다. 3시간 이후 stale, 범위 밖 strict 비교로 고온·저온·건조·과습 복수 위험을 생성하고 stale에서는 신규 push를 금지한다. 전역 weather off가 식물별 on을 우선하며 위험 진입 상태당 한 알림만 보낸다. 위치 capability event는 Loading 중에도 보존해 지연된 Firebase account identity와 join하고 UID별 철회 pending을 내구성 있게 재시도한다. Android 알림 identity는 32-bit hash 충돌을 해결해 영속화하며, scheduled refresh는 per-user in-flight/settled cursor, cancellable 10초 provider timeout, 7분 30초 deadline과 lease로 500명 이후까지 재개한다. 날씨 설정은 공통 알림 capability를 표시해 API 33 runtime 요청과 모든 지원 API의 앱 알림 설정 복구를 제공한다.
  ...
  Acceptance criteria (agent-executable): tests가 위치 허용/거부/철회 실패 후 resume·restart 재시도, delayed account emission과 load 중 permission/account 전환의 exact join, 수동 우선, API timeout·stale, 경계 equality safe, 복수 위험, 관리 콘텐츠 없음, 전역/식물별 알림 precedence와 dedupe, known notification hash collision/restart와 stable delivery identity 기반 ACTION_VIEW cold/warm/logged-out/activity·process recreation dedupe·distinct notification stack replacement·login target exact-once resume, claim 이후 fresh send-boundary clock 만료 차단, provider 실패 중 2:59→3:01 stale 재계산·scheduled completion clock·반복 실패·direct callable parity·day/timezone·기존 stale 보존, unavailable criteria ID의 transactional persistence·owner pruning·reload/recreation·concurrent plant/content 변경 차단, retained controller의 generation-safe Activity gateway rebind·회전/resume 중 permission/location exact-once continuation·same-UID owner/reconciliation/action cancellation-domain isolation·OS permission/app consent intent의 UI/접근성 분리와 explicit revoke 후 granted resume 불변성·UID별 server-authoritative exact-next consent command·same-payload replay·altered/stale/jump/MAX/float/negative 차단·concurrent device typed conflict의 explicit/automatic 공통 authoritative adoption·latest desired 보존·bounded exact-next convergence·conflict 후 response loss/recreation·UID switch/stale callback 격리·legacy exhausted granted/denied revoke-only recovery와 응답 유실 재시도·현재 위치 direct/scheduled 실행의 exact granted consent generation precondition·provider 중 revoke/실패/response loss/newer regrant typed abort와 snapshot/risk/alert 무변경·정상 transaction의 manual→device source 전환 및 conflict/응답 유실/재시작 수렴·수동 source 재전환·weather/collection/watering path-safe opaque ID 1/64/65/128 허용과 129/invalid 차단·late callback suppression·UID/account/process recreation, 반복 10초 timeout·1~2명 후 중단·restart·mixed failure·501+ scheduled user pagination/cursor wrap/concurrency 진행, 알림 granted/denied/permanent/API29~32/resume CTA를 검증한다. API 37 실제 Activity 계측은 application-owned Room/repository identity와 generation이 collection/detail/registration/watering/weather 각 route의 반복 회전 및 entry ID 복원 동안 유지되고, canonical logout에서 retained entry lifecycle이 `DESTROYED`가 되며 A→B cache 화면이 B owner만 노출하고 최종 runtime shutdown receipt 뒤 Room이 닫히는지 검증한다. Collection detail과 watering completion reconciliation을 포함해 Compose main coroutine에서 도달 가능한 Room cache/outbox query는 generated suspend DAO/transaction으로 실행하고 production과 같은 main-thread 금지 DB 설정에서 detail load, server-success local reconciliation, cancellation, account switch, database shutdown을 검증한다. Registration duplicate/completion navigation은 retained controller에 callback을 보존하지 않고 owner-scoped stable event identity로 전달하며, auth ownership을 nullable UID가 아닌 Restoring/Unknown/SignedOut/Authenticated(uid)로 구분한다. Restoring/Unknown은 pending event를 보류하고 confirmed SignedOut은 취소하며 같은 authenticated UID만 exact-once 전달한다. 두 번 회전 뒤 현재 NavController만 사용하고 recreation 중 emission·stale collector/auth emission·process restore 후 same UID/signed-out/different UID·delayed auth/login return·same/distinct identity·account switch/logout·ViewModel clear·callback graph reachability를 검증한다.
...
- [ ] 17. 분석·보안·데이터 수명주기 강화
  ...
  Acceptance criteria (agent-executable): analytics schema test와 log snapshot에 금지 필드가 없고, fake clock 23:59에는 임시 원본 존재·24:00 이후 삭제·대표 사진 보존, `gitleaks detect --no-banner`, dependency audit, App Check emulator tests가 통과한다. 모든 exported onCall은 auth 유무와 무관하게 App Check를 강제하고 missing/invalid token은 application validation 전에 거부하며 emulator unsigned debug token만 검증 생략 모드에서 handler에 도달한다. Apple session create는 app ID+secret-HMAC IP rate key로 10분당 10회 transaction limit을 적용하고, server-owned TTL/expiresAt, one-time callback/consume, direct Rules deny, expiresAt/name index와 200개 scheduled cleanup, replay/rate-limit/secret-redaction emulator 계약을 검증한다. Apple server가 호출하는 non-callable OAuth callback만 strict state/TTL/transaction 경계로 문서화한다. Generated `code id_token`/`form_post` authorize 계약은 standard success, first-login `user`, subsequent callback, provider error를 strict bounded schema로 수용하고 malformed/duplicate/oversized/mixed/unknown/prototype-polluted payload는 attachment 전에 거부한다. Callback `id_token`과 `user`는 검증된 code exchange 결과가 아니므로 신뢰하거나 저장하지 않으며 exchange의 issuer/audience/expiry/nonce 검증만 인증에 사용한다.
