...
- 상세 load와 account sync는 같은 App Check callable authoritative reader를 사용한다. Admin Firestore transaction이 단일 home과 해당 placement query를 한 snapshot에서 읽고 owner, exact revision lineage, operation ID, hash, plant count, bounded row count, contiguous z-order와 모든 placement revision을 검증한 schema-complete layout만 반환한다. missing home과 zero-placement home은 별도 typed 결과다. 모바일은 응답 필드와 count/owner/revision을 다시 검증하며 partial/malformed/cross-owner 응답은 transient inconsistent read로 실패시켜 기존 Room layout을 건드리지 않는다. save 또는 account deletion과 겹친 read는 한 시점의 전체 revision만 반환하고 다음 authoritative read에서 최신 revision/deletion을 채택한다.
- Mini-home ordering generation은 layout 존재 여부나 layout revision과 분리된 owner 전역 server epoch다. state 문서가 없는 최초 authoritative load는 같은 Firestore transaction에서 generation 1의 `DELETED/initial-missing` 또는 기존 home identity를 반영한 `ACTIVE` state를 먼저 만들고, 이후 save/delete가 그 epoch를 정확히 1씩 전진시킨다. callable과 Android는 generation 0을 authoritative 값으로 받지 않는다. Room v12→13이 기존 layout revision에서 추론한 watermark는 v13→14에서 명시적 `verified=false`가 되며 offline display에만 사용된다. 첫 schema-complete server layout/deletion은 숫자가 더 작아도 이 unverified watermark를 한 transaction에서 교체하고 `verified=true`로 전환한다. 그 이후에는 generation이 작은 응답은 `Ignored`, 같은 generation의 exact identity/content replay만 idempotent `Ignored`, mismatch는 `Conflict`, 큰 generation만 `Applied`다. 따라서 bootstrap 전에 시작한 늦은 응답도 verified state를 삭제·회귀시킬 수 없고, normal load와 account sync는 동일한 Room apply 결과의 현재 cache만 게시한다.
