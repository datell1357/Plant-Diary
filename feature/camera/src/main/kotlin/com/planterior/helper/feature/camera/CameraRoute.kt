...
fun CameraRoute(
    ...
    CameraScreen(
        ...
        onPicker = {
            if (controller.state is CameraFlowState.Capturing) controller.captureCancelled()
            controller.choosePicker()
        },
        ...
        onFlash = captureSession::toggleFlash,
        onReplace = controller::replacePhoto,
        onRetake = { controller.retakePhoto(context.currentCameraPermissionAfterRequest()) },
...
private class CameraCaptureSession {
    var action: () -> Unit = {}
    var flashAction: () -> Unit = {}

    fun capture() = action()

    fun toggleFlash() = flashAction()
}
            ...
            future.addListener(
                {
                    ...
                    session.flashAction = {
                        capture.flashMode =
                            if (capture.flashMode == ImageCapture.FLASH_MODE_ON) {
...
private fun Context.currentCameraPermissionAfterRequest(): CameraPermission =
    if (
        ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) ==
            PackageManager.PERMISSION_GRANTED
    ) {
